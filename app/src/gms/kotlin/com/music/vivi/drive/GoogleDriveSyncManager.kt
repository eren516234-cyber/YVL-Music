...
object GoogleDriveSyncManager {
    ...
    private const val BACKUP_FILE_NAME = "YVLMusicBackup.zip"
    ...
    private fun getSignedInAccountInternal(context: Context): GoogleSignInAccount? {
    ...
    fun getSignedInAccount(context: Context): DriveAccountInfo? {
        val account = getSignedInAccountInternal(context) ?: return null
        return DriveAccountInfo(email = account.email)
    }
    ...
    fun handleSignInResult(data: Intent?, context: Context): Result<DriveAccountInfo> {
        ...
        return try {
            ...
            val account = getSignedInAccount(context)
                ?: return Result.failure(Exception("Sign-in succeeded but no account found"))
    ...
    suspend fun uploadBackupZip(context: Context, backupFile: File): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            if (existingFileId != null) {
                val updateMeta = com.google.api.services.drive.model.File().apply { name = BACKUP_FILE_NAME }
    ...
    suspend fun getBackupTime(context: Context): Long? = withContext(Dispatchers.IO) {
        try {
            val account = getSignedInAccountInternal(context) ?: return@withContext null
            val driveService = getDriveService(context, account)
            val fileList = driveService.files().list()
                .setSpaces("appDataFolder")
                .setQ("name = '$BACKUP_FILE_NAME'")
                ...
                .execute()
            fileList.files.firstOrNull()?.modifiedTime?.value
        } catch (e: Exception) {
            null
        }
    }
